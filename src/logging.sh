...
# GLOBAL VARS
##################
declare -A __default_logger_state=([DEFAULT_LOG_LEVEL]=${LOG_LEVEL:-"WARNING"} [LOG_LEVEL]=${LOG_LEVEL:-"WARNING"} [DEFAULT_LOG_PREFIX]="1" [LOG_PREFIX]=${LOG_PREFIX:-"1"})
if [ -z "${__logger_state+x}" ]; then
	declare -A __logger_state
	for key in "${!__default_logger_state[@]}"; do
		__logger_state[$key]=${__default_logger_state[$key]}
	done
fi

declare -A __log_colors=([5]="bg-red" [4]="fg-red" [3]="fg-yellow" [2]="fg-blue" [1]="fg-cyan" [0]="fg-white")
...
function __format_log() {
	...
	case "${log_level_subidx}" in
		1)
			colored=$(color "${prefix}#${message}" "${__log_colors[${log_level_idx}]}")
			;;
		2)
			colored=$(color "${prefix}##${message}" "${__log_colors[${log_level_idx}]}")
			;;
		...
		6)
			...
			for item in "${message_array[@]}"; do
				local item_str="$(echo -en $(color "\n\t-${prefix}${item}" "${__log_colors[${log_level_idx}]}"))"
		...
		7)
			...
			local i=1
			for item in "${message_array[@]}"; do
				local item_str="$(echo -en $(color "\n\t${i}.${prefix}${item}" "${__log_colors[${log_level_idx}]}"))"
				colored="${colored}${item_str}"
				i=$((i + 1))
			done
			;;
...
function log() {
	...
	read -a current_global_log_level_array <<< "$(__resolve_log_level "${__logger_state[LOG_LEVEL]}")"
	...
	if [[ ! $current_global_log_level_idx > $log_level_idx ]]; then
		prefix=""
		if [[ ${__logger_state[LOG_PREFIX]} == "1" ]]; then
...
function log_off {
	__logger_state[LOG_LEVEL]="SILENT"
}
...
function log_level {
	...
	read -a log_level_array <<< "$(__resolve_log_level "${1}")"
	log_level_str=${log_level_array[0]}
	__logger_state[LOG_LEVEL]="$log_level_str"
}

function log_level_global {
	local log_level_str
	read -a log_level_array <<< "$(__resolve_log_level "${1}")"
	log_level_str=${log_level_array[0]}
	__logger_state[DEFAULT_LOG_LEVEL]="$log_level_str"
	__logger_state[LOG_LEVEL]="$log_level_str"
}

function log_on {
	__logger_state[LOG_LEVEL]=${__logger_state[DEFAULT_LOG_LEVEL]}
}

function log_prefix {
	__logger_state[LOG_PREFIX]="${1}"
}
...
function log_level_safe() {
	...
	local origin_log_level="${__logger_state[LOG_LEVEL]}"
	if [[ ${origin_log_level} == ${__logger_state[DEFAULT_LOG_LEVEL]} ]]; then
		log_level "${func_log_level}"
	fi
	local origin_log_prefix="${__logger_state[LOG_PREFIX]}"
	if [[ ${origin_log_prefix} == ${__logger_state[DEFAULT_LOG_PREFIX]} ]]; then
...
## SAMPLES/TESTS
###################
# echo "Default logging level ${__logger_state[DEFAULT_LOG_LEVEL]}"
# echo "Current logging level ${__logger_state[LOG_LEVEL]}"

# echo "explicitely set level to verbose (prefer usage of env variable LOG_LEVEL to apply on all your script)"
# __logger_state[LOG_LEVEL]=verbose
# # Basic usage
