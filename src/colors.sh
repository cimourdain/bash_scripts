...
declare -A __colors_classic__styles_ref=([NORMAL]=0 [BOLD]=1 [LOW_INTENSITY]=2 [UNDERLINE]=4 [BLINK]=5 [REVERSE]=7 [INVISIBLE]=8)
...
function __color_classic() {
	...
	for style_str in "${styles_arr[@]}"; do
		...
		if [[ ! ${__colors_classic__styles_ref[$style_str]-abc} = "abc" ]]; then
			style="${__colors_classic__styles_ref[$style_str]}"
		fi
